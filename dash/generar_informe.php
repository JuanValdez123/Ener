...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $consulta = "SELECT * FROM energia WHERE tiempo BETWEEN '$fecha_inicio' AND '$fecha_fin' AND trafo = $transformador";
   ...
   $pdf->Image("../logo1.png", $posicion_x, 0, 45);    //$pdf->Image("images/im1.png", 228, 15, 30,18);
    ...
    $pdf->Ln(20);
        ...
        $pdf->SetFont('Arial', 'B', 10);
         $pdf->Cell(42, 10, utf8_decode('Energía Activa(kWh)'), 1, 0, 'C');
        $pdf->Cell(48, 10, utf8_decode('Energía Reactiva(kVARh)'), 1, 0, 'C');
        $pdf->Cell(48, 10, utf8_decode('Energía Aparente(kVAh)'), 1, 0, 'C');
        $pdf->Cell(45, 10, 'Tiempo', 1, 1, 'C');

        // Agregar datos de la consulta
        $pdf->SetFont('Arial', '', 12);
        while ($fila = $resultado->fetch_assoc()) {
             $pdf->Cell(42, 10, $fila['energia_activa'], 1, 0, 'C');
            $pdf->Cell(48, 10, $fila['energia_reactiva'], 1, 0, 'C');
            $pdf->Cell(48, 10, $fila['energia_aparente'], 1, 0, 'C');
            $pdf->Cell(45, 10, $fila['tiempo'], 1, 1, 'C');

            // Sumar la energía activa
            $suma_energia_activa += $fila['energia_activa'];
        }

        // Mostrar la suma de la energía activa
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(138, 10, utf8_decode('Suma de Energía Activa(kWh):'), 1, 0, 'R');
        $pdf->Cell(45, 10, $suma_energia_activa, 1, 1, 'C');
